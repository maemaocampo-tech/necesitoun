...
function Admin() {
  ...
  const cargarPendientes = async () => {
    try {
      const q = query(collection(db, "tecnicos"), where("aprobado", "==", false));
    ...
    <div className="bg-white rounded-2xl p-4 shadow">
      ...
      {mostrarBotones && (
        <div className="flex gap-2 mt-3">
          <button onClick={() => aprobar(t.id)} className="flex-1 bg-green-500 text-white rounded-xl py-2 text-sm font-bold">Aprobar</button>
          <button onClick={() => rechazar(t.id)} className="flex-1 bg-red-400 text-white rounded-xl py-2 text-sm font-bold">Rechazar</button>
        </div>
      )}
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="flex gap-2 p-4">
        <button
          ...
          className={`flex-1 py-3 rounded-xl font-bold text-sm ${vista === "pendientes" ? "bg-blue-600 text-white" : "bg-white text-gray-600 shadow"}`}
        >
          Pendientes {pendientes.length > 0 && `(${pendientes.length})`}
        </button>
        <button
          onClick={() => setVista("recientes")}
          className={`flex-1 py-3 rounded-xl font-bold text-sm ${vista === "recientes" ? "bg-blue-600 text-white" : "bg-white text-gray-600 shadow"}`}
        >
          Ultimos 10
        </button>
      </div>
      ...
      <div className="px-4 flex flex-col gap-4">
        ...
        {!cargando && vista === "pendientes" && (
          <>
            {pendientes.length === 0 && <p className="text-gray-400 text-center mt-4">No hay pendientes</p>}
            {pendientes.map(t => <TecnicoCard key={t.id} t={t} mostrarBotones={true} />)}
          </>
        )}

        {!cargando && vista === "recientes" && (
          <>
            {recientes.length === 0 && <p className="text-gray-400 text-center mt-4">No hay tecnicos</p>}
            {recientes.map(t => <TecnicoCard key={t.id} t={t} mostrarBotones={false} />)}
          </>
        )}
      </div>
    </div>
